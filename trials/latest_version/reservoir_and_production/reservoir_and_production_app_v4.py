...
def calculate_geosteering_kpis(geosteering_df):
    ...
    kpis = {
        ...
        'lateral_length': geosteering_df[geosteering_df['inclination'] > 85]['md'].max() - 
                         geosteering_df[geosteering_df['inclination'] > 85]['md'].min() if any(geosteering_df['inclination'] > 85) else 0,
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H4(id='reservoir-quality', children='0%', style={'color': colors['success']}),
                    html.P('Res. Quality', style={'color': colors['text-secondary']}),
                    html.Small('Φ × So', style={'color': colors['text-secondary']})
                ], style={'textAlign': 'center'})
            ], style={'backgroundColor': colors['surface']})
        ], md=2)
    ], className='mb-4'),
...
@app.callback(
    [Output('processed-data-store', 'data'),
     ...
     Output('reservoir-quality', 'children'),
...
def process_geosteering_data(n_clicks, well_id, current_md, view_mode, steering_mode):
    if n_clicks == 0 and current_md == 0:
        return (None, '0 ft', '', '0°/0°', '', '0%', '', '0 ft', '', '0 ft/hr', '', '0%', None)
    ...
    res_quality = current_point['porosity'] * current_point['oil_saturation'] * 100
    res_quality_text = f"{res_quality:.1f}%"
...
# Add more callbacks for the side panel visualizations...
