# reservoir_geosteering_app.py - Updated with drilling parameters and DLS analysis
import dash
...
# ============================================================================
# GEOSTEERING DATA GENERATION FUNCTIONS (Updated with better DLS calculation)
# ============================================================================
...
def generate_geosteering_data(trajectory_df, layers, structure_map):
    ...
    for idx, point in trajectory_df.iterrows():
        ...
        # Higher DLS typically means slower ROP and higher torque
        dls_factor = 1 - min(point['dls'] / 10, 0.5)  # Reduce ROP with high DLS
        ...
        geosteering_data.append({
            ...
            'wob': wob,
            'rpm': rpm,
            'torque': wob * rpm / 100 + np.random.normal(0, 2)  # Simplified torque calculation
        })
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    dcc.Graph(id='dls-gauge', style={'height': '200px'}),
                    html.Hr(),
...
def update_tab_content(active_tab, processed_data):
    ...
    if active_tab == 'trajectory':
        # Create 3D trajectory visualization (same as before)
        ...
        # NEW: Drilling parameters vs time
        geo = pd.DataFrame(processed_data['geosteering'])
        current_idx = np.argmin(np.abs(geo['md'] - processed_data['current_md']))
        ...
        # Formation evaluation display (same as before)
        geo = pd.DataFrame(processed_data['geosteering'])
        current_idx = np.argmin(np.abs(geo['md'] - processed_data['current_md']))
        ...
        # Look-ahead prediction visualization (same as before)
