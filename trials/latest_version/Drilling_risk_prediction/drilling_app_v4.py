# drilling_risk_app.py - Complete Drilling Risk Prediction Dashboard with Offset Wells Map
import dash
...
def generate_offset_wells(rig_id, num_offset_wells=8):
    """Generate offset well data for comparison"""
    np.random.seed(hash(rig_id + "offset") % 2 ** 32)
    
    # Base location for the field (example: Permian Basin area)
    base_lat = 31.5 + np.random.uniform(-0.5, 0.5)
    base_lon = -102.5 + np.random.uniform(-0.5, 0.5)
    
    offset_wells = []
    
    # Current well location
    current_well = {
        'well_id': f'CURRENT-{rig_id}',
        'well_name': rigs_metadata[rig_id]['well_name'] if rig_id in rigs_metadata else 'Current Well',
        'lat': base_lat,
        'lon': base_lon,
        'is_current': True,
        'status': 'Drilling',
        'td': rigs_metadata[rig_id]['target_depth'] if rig_id in rigs_metadata else 12000,
        ...
        'lost_circ_events': 0,
        'kick_events': 0,
        'avg_rop': 45 + np.random.uniform(-10, 15),
        'total_npt_hours': 0,
        'formation': 'Wolfcamp A',
        'operator': rigs_metadata[rig_id]['operator'] if rig_id in rigs_metadata else 'Unknown'
    }
    offset_wells.append(current_well)
    
    # Generate offset wells around the current location
    formations = ['Wolfcamp A', 'Wolfcamp B', 'Spraberry', 'Bone Spring', 'Delaware Basin']
    statuses = ['Producing', 'Completed', 'P&A', 'Suspended']
    
    for i in range(num_offset_wells):
        # Random offset within ~2 miles (approximately 0.03 degrees)
        offset_lat = base_lat + np.random.uniform(-0.03, 0.03)
        offset_lon = base_lon + np.random.uniform(-0.03, 0.03)
        
        # Calculate approximate distance in feet (rough calculation)
        distance_ft = np.sqrt((offset_lat - base_lat)**2 + (offset_lon - base_lon)**2) * 364000
        
        # Historical well data
        spud_date = datetime.now() - timedelta(days=np.random.randint(180, 1200))
        drilling_days = np.random.randint(15, 60)
        completion_date = spud_date + timedelta(days=drilling_days)
        
        # Risk events that occurred
        stuck_pipe_events = np.random.choice([0, 0, 0, 1, 1, 2], p=[0.4, 0.2, 0.15, 0.15, 0.07, 0.03])
        lost_circ_events = np.random.choice([0, 0, 1, 1, 2, 3], p=[0.35, 0.25, 0.2, 0.1, 0.07, 0.03])
        kick_events = np.random.choice([0, 0, 0, 0, 1], p=[0.7, 0.15, 0.08, 0.05, 0.02])
        
        offset_well = {
            'well_id': f'OFFSET-{i+1:03d}',
            'well_name': f'WELL-{np.random.randint(100, 999)}-{chr(65+i)}',
            'lat': offset_lat,
            'lon': offset_lon,
            'is_current': False,
            'status': np.random.choice(statuses, p=[0.6, 0.25, 0.1, 0.05]),
            'td': 10000 + np.random.randint(0, 5000),
            'spud_date': spud_date,
            'completion_date': completion_date,
            'distance_ft': distance_ft,
            'total_incidents': stuck_pipe_events + lost_circ_events + kick_events,
            'stuck_pipe_events': stuck_pipe_events,
            'lost_circ_events': lost_circ_events,
            'kick_events': kick_events,
            'avg_rop': 35 + np.random.uniform(-10, 25),
            'total_npt_hours': (stuck_pipe_events * np.random.uniform(8, 48) + 
                               lost_circ_events * np.random.uniform(4, 24) + 
                               kick_events * np.random.uniform(12, 72)),
            'formation': np.random.choice(formations),
            'operator': np.random.choice(['DrillTech Solutions', 'Global Drilling Corp', 
                                         'Deepwater Operations', 'Continental Drilling',
                                         'Pioneer Resources', 'Apache Corp'])
        }
        offset_wells.append(offset_well)
    
    return offset_wells


def generate_offset_well_details(well_id):
    """Generate detailed historical data for a specific offset well"""
    np.random.seed(hash(well_id) % 2 ** 32)
    
    # Drilling parameters summary
    drilling_summary = {
        'avg_wob': 22 + np.random.uniform(-5, 8),
        'max_wob': 35 + np.random.uniform(-5, 10),
        'avg_rpm': 110 + np.random.uniform(-20, 30),
        'max_rpm': 160 + np.random.uniform(-10, 20),
        'avg_torque': 12 + np.random.uniform(-3, 8),
        'max_torque': 28 + np.random.uniform(-5, 12),
        'avg_spp': 2400 + np.random.uniform(-300, 400),
        'max_spp': 3200 + np.random.uniform(-200, 500),
        'avg_mud_weight': 10.2 + np.random.uniform(-0.5, 1.5),
        'max_mud_weight': 12.5 + np.random.uniform(-0.5, 2),
    }
    
    # Formation tops encountered
    formation_tops = [
        {'formation': 'Surface Casing', 'depth': 500 + np.random.randint(-100, 100), 'notes': 'No issues'},
        {'formation': 'Intermediate', 'depth': 3500 + np.random.randint(-200, 200), 'notes': 'Minor gas show'},
        {'formation': 'Spraberry', 'depth': 6500 + np.random.randint(-300, 300), 'notes': 'Increased ROP'},
        {'formation': 'Wolfcamp A', 'depth': 8500 + np.random.randint(-200, 200), 'notes': 'Lost circulation zone'},
        {'formation': 'Wolfcamp B', 'depth': 10500 + np.random.randint(-200, 200), 'notes': 'High pressure zone'},
    ]
    
    # Incident log
    incidents = []
    incident_types = ['Stuck Pipe', 'Lost Circulation', 'Tight Hole', 'Gas Kick', 'Equipment Failure', 'Twist Off']
    severities = ['Low', 'Medium', 'High', 'Critical']
    
    num_incidents = np.random.randint(0, 5)
    for i in range(num_incidents):
        incidents.append({
            'date': datetime.now() - timedelta(days=np.random.randint(200, 800)),
            'depth': np.random.randint(5000, 12000),
            'type': np.random.choice(incident_types),
            'severity': np.random.choice(severities, p=[0.4, 0.35, 0.2, 0.05]),
            'npt_hours': np.random.uniform(2, 48),
            'resolution': np.random.choice(['Worked free', 'LCM treatment', 'Increased MW', 
                                           'Short trip', 'Equipment repair', 'Fishing operation'])
        })
    
    # Lessons learned
    lessons = [
        "Maintain minimum 40 RPM while making connections in shale sections",
        "Pre-treat mud with LCM before entering Wolfcamp formation",
        "Keep ECD below 12.5 ppg to avoid losses in fractured zones",
        "Monitor torque trends closely below 8000 ft - differential sticking risk",
        "Flow check recommended every 500 ft in high pressure zones"
    ]
    ...
    return {
        ...
        'incidents': incidents,
        'lessons_learned': random.sample(lessons, min(3, len(lessons)))
    }
...
AI_RESPONSES = {
    ...
    'offset': [
        "Based on offset well analysis:\n• 3 nearby wells experienced lost circulation at similar depths\n• Average NPT in this area: 24 hours per well\n• Recommended mud weight range: 10.2-11.5 ppg\n• Most common issue: Differential sticking in Wolfcamp\n• Success factor: Pre-treating with LCM before problem zones",
        "Offset well comparison shows:\n• Wells within 1 mile had 40% stuck pipe incidents\n• Best performing well used 15% higher flow rate\n• Formation pressure increases below 9,500 ft\n• Recommend increasing MW by 0.3 ppg at 9,000 ft\n• Similar wells averaged 18 days to TD",
    ],
    'help': [
        "I can help you with:\n• Stuck pipe risk analysis & prevention\n• Lost circulation detection & management\n• Kick warning signs & well control\n• Drilling parameter optimization\n• Real-time risk assessment\n• Offset well comparison & lessons learned\n• Equipment recommendations\n\nJust ask me anything about your drilling operations!",
    ]
}
...
def get_ai_response(user_message):
    ...
    elif any(word in message_lower for word in ['offset', 'nearby', 'compare', 'historical', 'previous', 'neighbor']):
        return random.choice(AI_RESPONSES['offset'])
...
for rig_id in ['RIG-001', 'RIG-002', 'RIG-003', 'RIG-004', 'RIG-005']:
    ...
    rigs_offset_wells[rig_id] = generate_offset_wells(rig_id)
...
initial_chat = [
    {'text': "Hello! I'm AUTOMATA AI, your drilling risk prediction assistant. I'm monitoring real-time drilling parameters and can help you with stuck pipe prevention, lost circulation management, kick detection, drilling optimization, and offset well analysis. What would you like to know?", 'is_user': False}
]
...
app.layout = dbc.Container([
                ...
                dbc.CardBody([
                    dcc.Tabs(id='main-tabs', value='real-time', children=[
                        ...
                        dcc.Tab(label='Predictive Analysis', value='predictive',
                                style={'backgroundColor': colors['surface'], 'color': colors['text']},
                                selected_style={'backgroundColor': colors['primary'], 'color': colors['background']}),
                        dcc.Tab(label='Offset Wells Map', value='offset-map',
                                style={'backgroundColor': colors['surface'], 'color': colors['text']},
                                selected_style={'backgroundColor': colors['primary'], 'color': colors['background']})
                    ]),
                ...
                dbc.CardBody([
                    ...
                    html.Div([
                        ...
                        html.Div([
                            ...
                            dbc.Badge("Offset wells", color="info", className="me-1", id='quick-offset', style={'cursor': 'pointer'}),
                        ])
                    ])
                ])
            ], style={'backgroundColor': colors['surface'], 'border': f'1px solid {colors["surface-light"]}'})
    ...
    dbc.Modal([
        dbc.ModalHeader(dbc.ModalTitle(id='offset-modal-title', children="Offset Well Details"), 
                        close_button=True,
                        style={'backgroundColor': colors['surface'], 'color': colors['text']}),
        dbc.ModalBody(id='offset-modal-body', style={'backgroundColor': colors['surface']}),
        dbc.ModalFooter(
            dbc.Button("Close", id='close-offset-modal', className='ms-auto', n_clicks=0, color='primary'),
            style={'backgroundColor': colors['surface']}
        )
    ], id='offset-modal', size='xl', is_open=False, style={'color': colors['text']}),

    # Store components and interval for updates
    dcc.Store(id='processed-data-store'),
    dcc.Store(id='chat-history-store', data=initial_chat),
    dcc.Store(id='uploaded-files-store', data=[]),
    dcc.Store(id='selected-offset-well', data=None),
...
@app.callback(
    ...
    [Input('send-chat', 'n_clicks'),
     ...
     Input('quick-offset', 'n_clicks')],
    [State('chat-input', 'value'),
     State('chat-history-store', 'data')]
)
def update_chat(send_clicks, input_submit, q_stuck, q_lost, q_offset, user_message, chat_history):
    ...
    elif triggered_id == 'quick-offset':
        user_message = "What do the offset wells tell us about risk?"
...
def process_drilling_data(n_clicks, n_intervals, rig_id, time_window, threshold):
    ...
    processed_data = {
        ...
        'offset_wells': rigs_offset_wells[rig_id],
        'rig_id': rig_id
    }
...
def update_tab_content(active_tab, processed_data):
    ...
    if active_tab == 'real-time':
    ...
    elif active_tab == 'offset-map':
        # Create offset wells map
        offset_wells = processed_data['offset_wells']
        
        # Create the map figure
        fig = go.Figure()
        
        # Separate current well and offset wells
        current_well = [w for w in offset_wells if w['is_current']][0]
        offset_only = [w for w in offset_wells if not w['is_current']]
        
        # Color by total incidents for offset wells
        incident_colors = []
        for w in offset_only:
            if w['total_incidents'] == 0:
                incident_colors.append(colors['success'])
            elif w['total_incidents'] <= 2:
                incident_colors.append(colors['warning'])
            else:
                incident_colors.append(colors['danger'])
        
        # Add offset wells
        fig.add_trace(go.Scattermapbox(
            lat=[w['lat'] for w in offset_only],
            lon=[w['lon'] for w in offset_only],
            mode='markers+text',
            marker=dict(
                size=15,
            ...
            text=[w['well_name'] for w in offset_only],
            textposition='top right',
            textfont=dict(size=10, color=colors['text']),
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>" +
                "Status: %{customdata[1]}<br>" +
                "TD: %{customdata[2]:,.0f} ft<br>" +
                "Distance: %{customdata[3]:,.0f} ft<br>" +
                "Incidents: %{customdata[4]}<br>" +
                "Avg ROP: %{customdata[5]:.1f} ft/hr<br>" +
                "NPT: %{customdata[6]:.1f} hrs<br>" +
                "<extra></extra>"
            ),
            customdata=[[w['well_name'], w['status'], w['td'], w['distance_ft'], 
                        w['total_incidents'], w['avg_rop'], w['total_npt_hours']] for w in offset_only],
            name='Offset Wells'
        ))
        
        # Add current well (larger, different symbol)
        fig.add_trace(go.Scattermapbox(
            lat=[current_well['lat']],
            lon=[current_well['lon']],
            mode='markers+text',
            marker=dict(
                size=25,
            ...
            text=[f"CURRENT: {current_well['well_name']}"],
            textposition='top center',
            textfont=dict(size=12, color=colors['primary'], family='Arial Black'),
            hovertemplate=(
                "<b>🎯 CURRENT WELL</b><br>" +
                f"<b>{current_well['well_name']}</b><br>" +
                f"Status: {current_well['status']}<br>" +
                f"Target TD: {current_well['td']:,} ft<br>" +
                "<extra></extra>"
            ),
            name='Current Well'
        ))
        
        # Add circles to show distance ranges
        # This is approximated using the center point
        
        fig.update_layout(
            mapbox=dict(
                style='carto-darkmatter',
                center=dict(lat=current_well['lat'], lon=current_well['lon']),
                zoom=12
            ),
            height=600,
        ...
        total_incidents = sum(w['total_incidents'] for w in offset_only)
        avg_npt = np.mean([w['total_npt_hours'] for w in offset_only])
        wells_with_stuck = sum(1 for w in offset_only if w['stuck_pipe_events'] > 0)
        wells_with_losses = sum(1 for w in offset_only if w['lost_circ_events'] > 0)
        
        summary_cards = dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5(f"{len(offset_only)}", style={'color': colors['primary'], 'marginBottom': '0'}),
                        html.Small("Offset Wells", style={'color': colors['text-secondary']})
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5(f"{total_incidents}", style={'color': colors['warning'], 'marginBottom': '0'}),
                        html.Small("Total Incidents", style={'color': colors['text-secondary']})
                    ], style={'textAlign': 'center', 'padding': '10px'})
                ], style={'backgroundColor': colors['surface-light']})
            ], md=2),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5(f"{wells_with_stuck}", style={'color': colors['danger'], 'marginBottom': '0'}),
                        html.Small("Stuck Pipe Wells", style={'color': colors['text-secondary']})
                    ], style={'textAlign': 'center', 'padding': '10px'})
                ], style={'backgroundColor': colors['surface-light']})
            ], md=2),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5(f"{wells_with_losses}", style={'color': colors['secondary'], 'marginBottom': '0'}),
                        html.Small("Lost Circ Wells", style={'color': colors['text-secondary']})
                    ], style={'textAlign': 'center', 'padding': '10px'})
                ], style={'backgroundColor': colors['surface-light']})
            ], md=2),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5(f"{avg_npt:.1f} hrs", style={'color': colors['warning'], 'marginBottom': '0'}),
                        html.Small("Avg NPT/Well", style={'color': colors['text-secondary']})
                    ], style={'textAlign': 'center', 'padding': '10px'})
                ], style={'backgroundColor': colors['surface-light']})
            ], md=2),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5(f"{np.mean([w['avg_rop'] for w in offset_only]):.1f}", style={'color': colors['success'], 'marginBottom': '0'}),
                        html.Small("Avg ROP (ft/hr)", style={'color': colors['text-secondary']})
                    ], style={'textAlign': 'center', 'padding': '10px'})
                ], style={'backgroundColor': colors['surface-light']})
            ], md=2)
        ], className='mb-3')
        
        # Create offset wells table
        offset_table_data = []
        for w in offset_only:
            offset_table_data.append({
                'Well': w['well_name'],
                'Distance (ft)': f"{w['distance_ft']:,.0f}",
                'Status': w['status'],
                'TD (ft)': f"{w['td']:,}",
                'Formation': w['formation'],
                'Incidents': w['total_incidents'],
                'NPT (hrs)': f"{w['total_npt_hours']:.1f}",
                'Avg ROP': f"{w['avg_rop']:.1f}"
            })
        
        offset_table = dash_table.DataTable(
            id='offset-wells-table',
            data=offset_table_data,
            columns=[{'name': col, 'id': col} for col in offset_table_data[0].keys()],
            ...
            style_data_conditional=[
                {
                    'if': {'column_id': 'Incidents', 'filter_query': '{Incidents} > 2'},
                    'backgroundColor': colors['danger'],
                    'color': 'white'
                },
                {
                    'if': {'column_id': 'Incidents', 'filter_query': '{Incidents} > 0 && {Incidents} <= 2'},
                    'backgroundColor': colors['warning'],
                    'color': 'white'
                },
                {
                    'if': {'column_id': 'Incidents', 'filter_query': '{Incidents} = 0'},
        ...
        legend_info = html.Div([
            html.Span([
                html.I(className="fas fa-circle", style={'color': colors['success'], 'marginRight': '5px'}),
                "No Incidents  "
            ], style={'marginRight': '15px'}),
            html.Span([
                html.I(className="fas fa-circle", style={'color': colors['warning'], 'marginRight': '5px'}),
                "1-2 Incidents  "
            ], style={'marginRight': '15px'}),
            html.Span([
                html.I(className="fas fa-circle", style={'color': colors['danger'], 'marginRight': '5px'}),
                "3+ Incidents  "
            ], style={'marginRight': '15px'}),
            html.Span([
                html.I(className="fas fa-circle", style={'color': colors['primary'], 'marginRight': '5px', 'fontSize': '16px'}),
                "Current Well"
            ])
        ...
        return html.Div([
            html.H5([
                ...
                "Offset Wells Analysis - Click on a well in the table for detailed report"
            ], style={'color': colors['primary'], 'marginBottom': '15px'}),
            summary_cards,
            legend_info,
            dcc.Graph(figure=fig, id='offset-map-graph'),
            html.H6("Offset Wells Data (Click row for details)", 
                   style={'color': colors['text'], 'marginTop': '20px', 'marginBottom': '10px'}),
            offset_table
        ])


# Offset Well Modal Callback
@app.callback(
    [Output('offset-modal', 'is_open'),
     Output('offset-modal-title', 'children'),
     Output('offset-modal-body', 'children')],
    [Input('offset-wells-table', 'selected_rows'),
     Input('close-offset-modal', 'n_clicks')],
    [State('processed-data-store', 'data'),
     State('offset-modal', 'is_open')]
)
def toggle_offset_modal(selected_rows, close_clicks, processed_data, is_open):
    ctx = callback_context
    ...
    if triggered_id == 'close-offset-modal':
        return False, "", ""
    
    if triggered_id == 'offset-wells-table' and selected_rows and len(selected_rows) > 0:
        if not processed_data:
            return False, "", ""
        
        offset_wells = [w for w in processed_data['offset_wells'] if not w['is_current']]
        selected_well = offset_wells[selected_rows[0]]
        
        # Generate detailed data for this well
        well_details = generate_offset_well_details(selected_well['well_id'])
        ...
        modal_content = html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H6("Well Information", style={'color': colors['primary'], 'marginBottom': '15px'}),
                            html.P([html.Strong("Well Name: "), selected_well['well_name']], style={'margin': '5px 0'}),
                            html.P([html.Strong("Operator: "), selected_well['operator']], style={'margin': '5px 0'}),
                            html.P([html.Strong("Status: "), selected_well['status']], style={'margin': '5px 0'}),
                            html.P([html.Strong("Formation: "), selected_well['formation']], style={'margin': '5px 0'}),
                            html.P([html.Strong("Total Depth: "), f"{selected_well['td']:,} ft"], style={'margin': '5px 0'}),
                            html.P([html.Strong("Distance from Current: "), f"{selected_well['distance_ft']:,.0f} ft"], style={'margin': '5px 0'}),
                            html.P([html.Strong("Spud Date: "), selected_well['spud_date'].strftime("%Y-%m-%d")], style={'margin': '5px 0'}),
                            html.P([html.Strong("Completion Date: "), selected_well['completion_date'].strftime("%Y-%m-%d") if selected_well['completion_date'] else 'N/A'], style={'margin': '5px 0'}),
                        ])
                    ], style={'backgroundColor': colors['surface-light'], 'height': '100%'})
                ], md=4),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H6("Drilling Parameters Summary", style={'color': colors['primary'], 'marginBottom': '15px'}),
                            dbc.Row([
                                dbc.Col([
                                    html.P([html.Strong("Avg WOB: "), f"{well_details['drilling_summary']['avg_wob']:.1f} klbs"]),
                                    html.P([html.Strong("Max WOB: "), f"{well_details['drilling_summary']['max_wob']:.1f} klbs"]),
                                    html.P([html.Strong("Avg RPM: "), f"{well_details['drilling_summary']['avg_rpm']:.0f}"]),
                                    html.P([html.Strong("Max RPM: "), f"{well_details['drilling_summary']['max_rpm']:.0f}"]),
                                ], md=6),
                                                                dbc.Col([
                                    html.P([html.Strong("Avg Torque: "), f"{well_details['drilling_summary']['avg_torque']:.1f} klb-ft"]),
                                    html.P([html.Strong("Max Torque: "), f"{well_details['drilling_summary']['max_torque']:.1f} klb-ft"]),
                                    html.P([html.Strong("Avg SPP: "), f"{well_details['drilling_summary']['avg_spp']:.0f} psi"]),
                                    html.P([html.Strong("Max SPP: "), f"{well_details['drilling_summary']['max_spp']:.0f} psi"]),
                                ], md=6)
                            ]),
                            html.Hr(),
                            html.P([html.Strong("Avg Mud Weight: "), f"{well_details['drilling_summary']['avg_mud_weight']:.2f} ppg"]),
                            html.P([html.Strong("Max Mud Weight: "), f"{well_details['drilling_summary']['max_mud_weight']:.2f} ppg"]),
                        ])
                    ], style={'backgroundColor': colors['surface-light'], 'height': '100%'})
                ], md=4),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H6("Incident Summary", style={'color': colors['primary'], 'marginBottom': '15px'}),
                            dbc.Row([
                                dbc.Col([
                                    html.Div([
                                        html.H3(f"{selected_well['total_incidents']}", 
                                               style={'color': colors['danger'] if selected_well['total_incidents'] > 2 else colors['warning'] if selected_well['total_incidents'] > 0 else colors['success'], 
                                                      'marginBottom': '0'}),
                                        html.Small("Total Incidents", style={'color': colors['text-secondary']})
                                    ], style={'textAlign': 'center'})
                                ], md=6),
                                dbc.Col([
                                    html.Div([
                                        html.H3(f"{selected_well['total_npt_hours']:.1f}", 
                                               style={'color': colors['warning'], 'marginBottom': '0'}),
                                        html.Small("NPT Hours", style={'color': colors['text-secondary']})
                                    ], style={'textAlign': 'center'})
                                ], md=6)
                            ], className='mb-3'),
                            html.Hr(),
                            html.Div([
                                html.Span([
                                    html.I(className="fas fa-link", style={'marginRight': '8px', 'color': colors['warning']}),
                                    f"Stuck Pipe: {selected_well['stuck_pipe_events']}"
                                ], style={'display': 'block', 'marginBottom': '8px'}),
                                html.Span([
                                    html.I(className="fas fa-tint-slash", style={'marginRight': '8px', 'color': colors['primary']}),
                                    f"Lost Circulation: {selected_well['lost_circ_events']}"
                                ], style={'display': 'block', 'marginBottom': '8px'}),
                                html.Span([
                                    html.I(className="fas fa-arrow-up", style={'marginRight': '8px', 'color': colors['danger']}),
                                    f"Kick Events: {selected_well['kick_events']}"
                                ], style={'display': 'block'}),
                            ]),
                            html.Hr(),
                            html.P([html.Strong("Avg ROP: "), f"{selected_well['avg_rop']:.1f} ft/hr"], 
                                  style={'color': colors['success']})
                        ])
                    ], style={'backgroundColor': colors['surface-light'], 'height': '100%'})
                ], md=4)
                    ...
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            dash_table.DataTable(
                                data=well_details['formation_tops'],
                                columns=[
                                    {'name': 'Formation', 'id': 'formation'},
                                    {'name': 'Depth (ft)', 'id': 'depth'},
                                    {'name': 'Notes', 'id': 'notes'}
                                ],
                                style_cell={
                                    'textAlign': 'left',
                            ...
                            html.Div([
                                dash_table.DataTable(
                                    data=[{
                                        ...
                                        'Depth': f"{inc['depth']:,} ft",
                                        'Type': inc['type'],
                                        'Severity': inc['severity'],
                                        'NPT': f"{inc['npt_hours']:.1f} hrs",
                                        'Resolution': inc['resolution']
                                    } for inc in well_details['incidents']],
                                    columns=[
                                        {'name': 'Date', 'id': 'Date'},
                                        {'name': 'Depth', 'id': 'Depth'},
                                        {'name': 'Type', 'id': 'Type'},
                                ...
                                ) if well_details['incidents'] else html.P("No incidents recorded", 
                    ...
                    dbc.Card([
                        dbc.CardBody([
                            html.H6([
                                ...
                                "Lessons Learned & Recommendations for Current Well"
                            ], style={'color': colors['primary'], 'marginBottom': '15px'}),
                            html.Ul([
                                html.Li(lesson, style={'marginBottom': '10px', 'color': colors['text']}) 
                                for lesson in well_details['lessons_learned']
                            ], style={'paddingLeft': '20px'}),
                            html.Hr(),
                            html.Div([
                                html.H6("AI-Generated Risk Assessment Based on This Offset Well:", 
                                       style={'color': colors['warning'], 'marginBottom': '10px'}),
                                html.P([
                                    f"Based on the drilling history of {selected_well['well_name']}, located ",
                                    f"{selected_well['distance_ft']:,.0f} ft from your current location, ",
                                    f"I recommend the following precautions:"
                                ], style={'color': colors['text-secondary']}),
                                html.Ul([
                                    html.Li([
                                        html.Strong("Stuck Pipe Risk: ", style={'color': colors['warning']}),
                                        "High" if selected_well['stuck_pipe_events'] > 0 else "Low",
                                        f" - This well experienced {selected_well['stuck_pipe_events']} stuck pipe event(s). ",
                                        "Maintain rotation and circulation when in similar formations." if selected_well['stuck_pipe_events'] > 0 else ""
                                    ], style={'marginBottom': '8px'}),
                                    html.Li([
                                        html.Strong("Lost Circulation Risk: ", style={'color': colors['primary']}),
                                        "High" if selected_well['lost_circ_events'] > 0 else "Low",
                                        f" - {selected_well['lost_circ_events']} lost circulation event(s) recorded. ",
                                        "Pre-treat with LCM before entering fractured zones." if selected_well['lost_circ_events'] > 0 else ""
                                    ], style={'marginBottom': '8px'}),
                                    html.Li([
                                        html.Strong("Kick Risk: ", style={'color': colors['danger']}),
                                        "High" if selected_well['kick_events'] > 0 else "Low",
                                        f" - {selected_well['kick_events']} kick event(s) recorded. ",
                                        "Monitor gas readings closely and maintain proper mud weight." if selected_well['kick_events'] > 0 else ""
                                    ], style={'marginBottom': '8px'}),
                                ], style={'color': colors['text'], 'paddingLeft': '20px'}),
                                html.P([
                                    html.Strong("Expected NPT: "),
                                    f"Based on offset well performance, anticipate {selected_well['total_npt_hours'] * 0.8:.1f} - {selected_well['total_npt_hours'] * 1.2:.1f} hours of NPT in similar conditions."
                                ], style={'color': colors['text-secondary'], 'marginTop': '15px'})
                            ], style={
                                'backgroundColor': colors['surface'],
                                'padding': '15px',
                                'borderRadius': '10px',
        ...
        return True, f"📊 Offset Well Report: {selected_well['well_name']}", modal_content
...
def update_risk_heatmap(processed_data):
    ...
    # Add annotations for low values (outside the bar)
    ...
    fig.update_layout(
        xaxis=dict(range=[0, 110], showgrid=False, title='Risk %'),
...
def update_recommendations(processed_data):
    ...
    risks = processed_data['risks']
    offset_wells = processed_data.get('offset_wells', [])
    
    # Calculate offset well statistics for enhanced recommendations
    offset_only = [w for w in offset_wells if not w.get('is_current', False)]
    total_stuck_events = sum(w.get('stuck_pipe_events', 0) for w in offset_only)
    total_loss_events = sum(w.get('lost_circ_events', 0) for w in offset_only)

    recommendations = []

    if risks['stuck_pipe'] > 40 or total_stuck_events > 2:
        recommendations.append({
            'Risk': 'Stuck Pipe',
            'Level': 'High' if risks['stuck_pipe'] > 70 or total_stuck_events > 3 else 'Medium',
            'Primary Action': 'Reduce WOB to 15-20 klbs, increase RPM',
            'Secondary': 'Pump viscous sweep, short trip',
            'Offset Data': f'{total_stuck_events} events in nearby wells',
            'Priority': '1'
        })

    if risks['lost_circulation'] > 35 or total_loss_events > 2:
        recommendations.append({
            'Risk': 'Lost Circulation',
            'Level': 'High' if risks['lost_circulation'] > 65 or total_loss_events > 4 else 'Medium',
            'Primary Action': 'Add LCM (15-25 ppb), reduce flow rate',
            'Secondary': 'Reduce mud weight by 0.3-0.5 ppg',
            'Offset Data': f'{total_loss_events} events in nearby wells',
            'Priority': '2'
        })
    ...
    if risks['kick'] > 30:
        recommendations.append({
            ...
            'Offset Data': 'Monitor gas trends',
            'Priority': '1'
        })
    ...
    if risks['wellbore_instability'] > 40:
        recommendations.append({
            ...
            'Offset Data': 'Check offset mud programs',
            'Priority': '3'
        })
    ...
    if not recommendations:
        recommendations.append({
            ...
            'Offset Data': 'No significant issues in offsets',
            'Priority': '4'
        })
